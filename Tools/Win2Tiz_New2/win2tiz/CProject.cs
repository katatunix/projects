...
namespace win2tiz
{
	class CProject
	{
		public static List<TCommand> load(
			CGccConfig gccConfig, XmlNode projectNode, AProject vcProject,
			...
			out string projectNameSpec)
		{
			...
			{
				prjWorkingDir = CUtils.combinePath(slnWorkingDir, outputFolderPath) + "\\" + projectName;
			}
			...
			#region Get info from XML file
			{
				foreach (XmlNode childNode in projectNode.ChildNodes)
				{
					if (childNode.Name == KXml.s_kMacroTag)
					{
						...
						{
							USE_ADDITIONAL_INCLUDE_DIRECTORIES_FROM_VS = CXmlUtils.convertString2Bool(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
						}
						else if (macroName == "USE_EXCLUDEFROMBUILD_VS_FLAG")
						{
							USE_EXCLUDEFROMBUILD_VS_FLAG = CXmlUtils.convertString2Bool(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
						}
						else if (macroName == "INCLUDE_PATHS")
						{
							INCLUDE_PATHS = CXmlUtils.splitXmlValue(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
							for (int i = 0; i < INCLUDE_PATHS.Length; i++)
							{
								INCLUDE_PATHS[i] = CUtils.combinePath(vcDir, INCLUDE_PATHS[i]);
							}
						}
						else if (macroName == "LINK_PATHS")
						{
							LINK_PATHS = CXmlUtils.splitXmlValue(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
							for (int i = 0; i < LINK_PATHS.Length; i++)
							{
								// note: choice
								//LINK_PATHS[i] = CUtils.combinePath(slnWorkingDir, LINK_PATHS[i]);
								LINK_PATHS[i] = CUtils.combinePath(vcDir, LINK_PATHS[i]);
							}
						}
						else if (macroName == "DEFINES")
						{
							DEFINES = CXmlUtils.splitXmlValue(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
						}
						else if (macroName == "LDLIBS")
						{
							LDLIBS = CXmlUtils.splitXmlValue(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
						}
						else if (macroName == "LDFLAGS")
						{
							LDFLAGS = CXmlUtils.splitXmlValue(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
						}
						else if (macroName == "CFLAGS")
						{
							CFLAGS = CXmlUtils.splitXmlValue(
								gccConfig.getValueEscape(CXmlUtils.getXmlValue(childNode, isReleaseMode))
							);
						}
						#endregion
						...
						foreach (XmlNode fileNode in childNode.ChildNodes)
						{
							if (fileNode.Name == KXml.s_kFileTag)
							{
								...
								fileSpecificList.Add(new TFileSpecific(
									...
									cflags	== null ? null : CXmlUtils.splitXmlValue(gccConfig.getValueEscape(cflags)),
									defines	== null ? null : CXmlUtils.splitXmlValue(gccConfig.getValueEscape(defines))
								));
							}
						}
						#endregion
					...
					else if (childNode.Name == KXml.s_kAddSourceFileToProjectTag)
					{
						...
						foreach (XmlNode iNode in childNode.ChildNodes)
						{
							if (iNode.Name == KXml.s_kFileTag)
							{
								...
								name = CUtils.combinePathSimple(vcDir, name);

								string[] matchedFiles = CUtils.getFilesWithWildcard(name);
								foreach (string f in matchedFiles)
								{
									// f can be a non full path (e.g. e:\a\b\c\..\d.cpp
									addSourceFileToProjectList.Add( CUtils.getFullPath(f) );
								}
							}
						}
						#endregion
					...
					else if (childNode.Name == KXml.s_kUnityBuildsTag)
					{
						...
						foreach (XmlNode nodeAutoGenerated in childNode.ChildNodes)
						{
							...
							unityBuildsNumber = CXmlUtils.convertString2Int(att.Value);
			...
			#region Get source files from VC project
			{
				...
				foreach (string sourceFilePath in vcFiles)
				{
					...
					{
						if (CUtils.checkPatternFile(sourceFilePath, pattern, vcDir))
						{
							ignored = true;
							break;
						}
					...
					if (!CUtils.checkPathExistInList(sourceFilePath, sourceFilePathList))
					{
			...
			#region addSourceFileToProjectList to sourceFilePathList
			{
				foreach (string source in addSourceFileToProjectList)
				{
					...
					if (!CUtils.checkPathExistInList(source, sourceFilePathList))
					{
			...
			if (unityBuildsNumber > 0 && sourceFilePathList.Count > unityBuildsNumber)
			{
				...
				{
					bool ub = true;
					if ( !CUtils.isCppExt(Path.GetExtension(sourceFilePathList[i])) )
					{
					...
					else
					{
						...
						{
							if (CUtils.checkPatternFile(sourceFilePathList[i], fs.name, vcDir))
							{
								ub = false;
								break;
							}
						...
						if (ub)
						{
							...
							{
								if (CUtils.checkPatternFile(sourceFilePathList[i], pattern, vcDir))
								{
									ub = false;
									break;
								}
				...
				if (ubList.Count > 0)
				{
					...
					for (int g = 0; g < unityBuildsNumber; g++)
					{
						...
						string stringToWrite = "// nghia.buivan@gameloft.com - This file was auto generated - DO NOT EDIT\r\n";
						for (int k = left; k <= right; k++)
						{
							string rel = CUtils.getRelativePath(ubList[k], prjWorkingDir);
			...
			#region DEFINES
			{
				...
				{
					gccDefines += " " + CUtils.makeGccDefinesString(gccConfig.get_DEFINES()).Trim();
					gccDefines = gccDefines.Trim();
				}
				if (DEFINES != null)
				{
					gccDefines += " " + CUtils.makeGccDefinesString(DEFINES).Trim();
					gccDefines = gccDefines.Trim();
				}
			}
			#endregion
			...
			#region INCLUDE_PATHS
			{
				...
				{
					gccIncludePaths += " " + CUtils.makeGccIncludePathsString(INCLUDE_PATHS).Trim();
				...
				if (USE_ADDITIONAL_INCLUDE_DIRECTORIES_FROM_VS)
				{
					...
					if (incs != null)
					{
						gccIncludePaths += " " + CUtils.makeGccIncludePathsString(incs.ToArray()).Trim();
						gccIncludePaths = gccIncludePaths.Trim();
					}
				}
				if (gccConfig.get_INCLUDE_PATHS() != null)
				{
					gccIncludePaths += " " + CUtils.makeGccIncludePathsString(gccConfig.get_INCLUDE_PATHS()).Trim();
					gccIncludePaths = gccIncludePaths.Trim();
				}
			}
			#endregion
			...
			#region CFLAGS
			{
				...
				{
					gccCflags += " " + CUtils.makeGccItemsString(gccConfig.get_CFLAGS()).Trim();
					gccCflags = gccCflags.Trim();
				}

				if (CFLAGS != null)
				{
					gccCflags += " " + CUtils.makeGccItemsString(CFLAGS).Trim();
					gccCflags = gccCflags.Trim();
				}
			}
			#endregion
			...
			#region Check through sourceFilePathList for dependencies and make compile commands list
			{
				...
				for (int i = 0; i < sourceCount; i++)
				{
					...
					foreach (TFileSpecific fs in fileSpecificList)
					{
						if (CUtils.checkPatternFile(sourceFilePath, fs.name, vcDir))
						{
							if (fs.cflags != null)
							{
								if (_gccCflags.Length > 0) _gccCflags += " ";
								_gccCflags += CUtils.makeGccItemsString(fs.cflags);
							...
							if (fs.defines != null)
							{
								...
								_gccDefines += CUtils.makeGccDefinesString(fs.defines);
					...
					string objFilePathFull = CUtils.combinePath(prjWorkingDir, objFileName);

					// Now create compile cmd
					if (CUtils.isCppExt(sourceFileExt))
					{
				...
				case EProjectType.eExecutable:
				{
					...
					if (relink)
					{
						...
						#region LDLIBS
						{
							...
							if (LDLIBS != null)
							{
								gccLdlibs += " " + CUtils.makeGccItemsString(LDLIBS).Trim();
								gccLdlibs = gccLdlibs.Trim();
							}
							if (gccConfig.get_LDLIBS() != null)
							{
								gccLdlibs += " " + CUtils.makeGccItemsString(gccConfig.get_LDLIBS()).Trim();
								gccLdlibs = gccLdlibs.Trim();
							}
						}
						#endregion
						...
						#region LINK_PATHS
						{
							if (depProjectInfos != null && depProjectInfos.Count > 0)
							{
								...
								gccLinkPaths += " " + CUtils.makeGccLinkPathsString(paths).Trim();
								gccLinkPaths = gccLinkPaths.Trim();
							}
							if (LINK_PATHS != null)
							{
								gccLinkPaths += " " + CUtils.makeGccLinkPathsString(LINK_PATHS).Trim();
								gccLinkPaths = gccLinkPaths.Trim();
							}
							if (gccConfig.get_LINK_PATHS() != null)
							{
								gccLinkPaths += " " + CUtils.makeGccLinkPathsString(gccConfig.get_LINK_PATHS()).Trim();
								gccLinkPaths = gccLinkPaths.Trim();
							}
						}
						#endregion
						...
						{
							gccLdflags += " " + CUtils.makeGccItemsString(gccConfig.get_LDFLAGS()).Trim();
							gccLdflags = gccLdflags.Trim();
						}
						if (LDFLAGS != null)
						{
							gccLdflags += " " + CUtils.makeGccItemsString(LDFLAGS).Trim();
							gccLdflags = gccLdflags.Trim();
						}
						#endregion
		...
		public static readonly string s_kObjFileExt = ".o";
		public static readonly string s_kDepFileExt = ".d";
