...
namespace win2tiz
{
	class CMongccServerSocket : AMongccSocket
	{
			...
			while (true)
			{
				...
				switch (msg.getTypeEnum())
				{
					...
					case EMessageType.eCompileRequest:
					{
						...
						handleMessageCompileCommand(clientId, sessionFolderPath, msg);
						setState(EState.eWaiting);
						break;
					}
				}
		...
		private void handleMessageCompileCommand(string clientId, string sessionFolderPath, CMessage msg)
		{
			CMessageCompileRequest msgCC = new CMessageCompileRequest(msg);

			string cmd = msgCC.getCmd();
